...
class SurveyShuffle extends \ExternalModules\AbstractExternalModule {
    ...
    /** Read any stored sequence (random mode only persists via sequence_field). */
    private function ss_read_sequence(string $record, int $event_id, ?string $sequence_field, ?int $sequence_field_event): array {
        if (empty($sequence_field)) return [];
        $ev = $sequence_field_event ?? $event_id;
        $d  = \REDCap::getData('array', $record, $sequence_field, $ev);
        $v  = trim((string)($d[$record][$ev][$sequence_field] ?? ''));
        return $v === '' ? [] : array_map('trim', explode(', ', $v));
    }

    /**
     * Return the one-and-done shuffled order.
     * - If a sequence already exists -> returns it.
     * - If none exists and $persist_if_missing is true AND sequence_field is configured AND we're at entry -> creates, saves, and returns it.
     * - Otherwise returns a fresh (non-persisted) random order.
     */
    private function ss_get_or_create_sequence(
        string $record,
        int $event_id,
        array $shuffle_instruments,
        ?string $sequence_field,
        ?int $sequence_field_event,
        ?int $shuffle_number,
        bool $persist_if_missing,
        bool $is_entry_instrument
    ): array {
        // Use stored sequence if present
        $stored = $this->ss_read_sequence($record, $event_id, $sequence_field, $sequence_field_event);
        if (!empty($stored)) return $stored;

        // Build a random order (respect limit)
        $order = array_values($shuffle_instruments);
        shuffle($order);
        if (!is_null($shuffle_number) && is_numeric($shuffle_number) && $shuffle_number > 0 && $shuffle_number < count($order)) {
            $order = array_slice($order, 0, (int)$shuffle_number);
        }

        // Persist once only if: sequence_field exists, persistence requested, and we're at entry
        if ($persist_if_missing && !empty($sequence_field) && $is_entry_instrument) {
            $ev = $sequence_field_event ?? $event_id;
            $check = \REDCap::getData('array', $record, $sequence_field, $ev);
            $exists = trim((string)($check[$record][$ev][$sequence_field] ?? ''));
            if ($exists === '') {
                \REDCap::saveData('array', [
                    $record => [ $ev => [ $sequence_field => implode(', ', $order) ] ]
                ]);
                return $order; // authoritative this request
            }
            // Another path won the race: reload authoritative value
            return $this->ss_read_sequence($record, $event_id, $sequence_field, $sequence_field_event);
        }

        // No persistence (either no field or not at entry): use ephemeral order
        return $order;
    }
        ...
        for ($i = 0; $i < count($configs); $i++) {
            ...
            if ($entry_survey == $instrument) {
                ...
                if (is_null($shuffle_type) || $shuffle_type == 'random') {
                    $shuffle_array = $this->ss_get_or_create_sequence(
                        $record,
                        $event_id,
                        (array)$shuffle_instruments,
                        $sequence_field,
                        $sequence_field_event,
                        (is_numeric($shuffle_number) ? (int)$shuffle_number : null),
                        /* persist_if_missing */ true, // persist only if sequence_field is configured
                        /* is_entry_instrument */ true
                    );
